...
Check: status is RECEIVED or FAILED? (idempotency guard against duplicate task enqueue; PROCESSED, DEAD_LETTER and CANCELLED are no-ops; an event whose integration is DISCONNECTED becomes CANCELLED)
   ↓
adapter.normalize(parsed_payload) -> SaleCreated
   ↓
Customer get-or-create (merchant_id, phone) — only when the sale supplies a phone
   ↓
Transaction create (unique on location_id + external_transaction_id; customer may be null)
   ↓
...
|---|---|
| `parse()` throws (malformed payload) | `IntegrationEvent.status = FAILED`, safe `error_code` + `error_message` stored (never exception text or payload values) |
| `normalize()` throws | same as above |
| Sale has no customer phone | not a failure: `Transaction` recorded with no `Customer`, `status = PROCESSED`; never review-eligible |
| Supplied phone is not valid E.164 | `FAILED`, `error_code = INVALID_PHONE` |
| Integration disconnected | pending `RECEIVED`/`FAILED` events → `CANCELLED` (terminal, never retried) |
...
| Unexpected exception | `FAILED`, `error_code = PROCESSING_ERROR`, generic message; retried by `retry_failed_events` with backoff |
...
- Multiple Celery workers may pick up retries of the same failed event; the `IntegrationEvent` status guard (only `RECEIVED`/`FAILED` are processed), combined with the `Transaction` row lock and the unique constraints above, makes concurrent processing of the same event — or of two different events touching the same transaction — safe without a separate distributed lock.
